...
//--------------------------------------------------------------------------------------
static inline uint32_t MakeFourCC(char ch0, char ch1, char ch2, char ch3);
...
extern bool load_dds_image_asset_header_from_input_stream(load_asset_input_stream *input_stream, LOAD_IMAGE_ASSET_HEADER *image_asset_header, size_t *image_asset_data_offset)
{
    ...
    if ((*dwMagicNumber) != MakeFourCC('D', 'D', 'S', ' '))
    {
        return false;
    }
    ...
    PAL_ASSET_IMAGE_TYPE res_dim = static_cast<PAL_ASSET_IMAGE_TYPE>(-1);
    ...
    if (NULL != d3d10ext)
    {
        ...
        case DDS_RESOURCE_DIMENSION_TEXTURE1D:
        {
            ...
            res_dim = PAL_ASSET_IMAGE_TYPE_1D;
        }
        break;
        case DDS_RESOURCE_DIMENSION_TEXTURE2D:
        {
            ...
            res_dim = PAL_ASSET_IMAGE_TYPE_2D;
        }
        break;
        case DDS_RESOURCE_DIMENSION_TEXTURE3D:
        {
            ...
            res_dim = PAL_ASSET_IMAGE_TYPE_3D;
        }
        break;
        default:
            return false;
    ...
    case DDS_FORMAT_R8G8B8A8_UNORM:
        image_asset_header->format = PAL_ASSET_IMAGE_FORMAT_R8G8B8A8_UNORM;
        break;
    case DDS_FORMAT_BC7_UNORM:
        image_asset_header->format = PAL_ASSET_IMAGE_FORMAT_BC7_UNORM_BLOCK;
        break;
    default:
        // TODO: support more format
        assert(false);
        image_asset_header->format = static_cast<PAL_ASSET_IMAGE_FORMAT>(-1);
        return false;
    }
...
{
#ifndef NDEBUG
    LOAD_IMAGE_ASSET_HEADER image_asset_header_for_validate;
    size_t image_asset_data_offset_for_validate;
    if (!load_dds_image_asset_header_from_input_stream(input_stream, &image_asset_header_for_validate, &image_asset_data_offset_for_validate))
    {
        return false;
    }
    ...
    if (image_asset_header->mip_levels > 15U) // D3D12_REQ_MIP_LEVELS
    {
        return false;
    }

    switch (image_asset_header->type)
    {
    case PAL_ASSET_IMAGE_TYPE_1D:
        if ((image_asset_header->array_layers > 2048U) || // D3D12_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION
            (image_asset_header->width > 16384U))         // D3D12_REQ_TEXTURE1D_U_DIMENSION
        {
            return false;
        }
        break;

    case PAL_ASSET_IMAGE_TYPE_2D:
        if (!(image_asset_header->is_cube_map))
        {
            if ((image_asset_header->array_layers > 2048U) || // D3D12_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION
                (image_asset_header->width > 16384U) ||       // D3D12_REQ_TEXTURECUBE_DIMENSION
                (image_asset_header->height > 16384U))        // D3D12_REQ_TEXTURECUBE_DIMENSION
            {
                return false;
            }
        }
        else
        {
            ...
            if ((image_asset_header->array_layers > 2048U) || // D3D12_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION
                (image_asset_header->width > 16384U) ||       // D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION
                (image_asset_header->height > 16384U))        // D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION
            {
                return false;
            }
        }
    ...
    case PAL_ASSET_IMAGE_TYPE_3D:
        if ((image_asset_header->array_layers > 1U) ||
            (image_asset_header->width > 2048U) ||  // D3D12_REQ_TEXTURE3D_U_V_OR_W_DIMENSION
            (image_asset_header->height > 2048U) || // D3D12_REQ_TEXTURE3D_U_V_OR_W_DIMENSION
            (image_asset_header->depth > 2048U))    // D3D12_REQ_TEXTURE3D_U_V_OR_W_DIMENSION
        {
            return false;
        }
        break;
    ...
    // Create the texture
    size_t const numberOfResources = ((PAL_ASSET_IMAGE_TYPE_3D == image_asset_header->type) ? static_cast<size_t>(1U) : static_cast<size_t>(image_asset_header->array_layers)) * static_cast<size_t>(image_asset_header->mip_levels) * static_cast<size_t>(numberOfPlanes);
    ...
    for (uint32_t planeSlice = 0; planeSlice < 1U; ++planeSlice)
    {
        ...
        for (uint32_t arraySlice = 0; arraySlice < image_asset_header->array_layers; ++arraySlice)
        {
            ...
            for (uint32_t mipSlice = 0; mipSlice < image_asset_header->mip_levels; ++mipSlice)
            {
                ...
                uint32_t dstSubresource = load_image_asset_calculate_subresource_index(mipSlice, arraySlice, planeSlice, image_asset_header->mip_levels, image_asset_header->array_layers);
                ...
                if (inputSliceSize == subresource_memcpy_dests[dstSubresource].output_slice_pitch && inputRowSize == subresource_memcpy_dests[dstSubresource].output_row_pitch)
                {
                    ...
                    ptrdiff_t BytesRead = input_stream->read(reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(staging_buffer_base) + subresource_memcpy_dests[dstSubresource].staging_buffer_offset), inputSliceSize * inputNumSlices);
                ...
                else if (inputRowSize == subresource_memcpy_dests[dstSubresource].output_row_pitch)
                {
                    ...
                    for (size_t z = 0; z < inputNumSlices; ++z)
                    {
                        ...
                        ptrdiff_t BytesRead = input_stream->read(reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(staging_buffer_base) + (subresource_memcpy_dests[dstSubresource].staging_buffer_offset + subresource_memcpy_dests[dstSubresource].output_slice_pitch * z)), inputSliceSize);
                ...
                else
                {
                    ...
                    for (size_t z = 0; z < inputNumSlices; ++z)
                    {
                        for (size_t y = 0; y < inputNumRows; ++y)
                        {
                            ...
                            ptrdiff_t BytesRead = input_stream->read(reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(staging_buffer_base) + (subresource_memcpy_dests[dstSubresource].staging_buffer_offset + subresource_memcpy_dests[dstSubresource].output_slice_pitch * z + subresource_memcpy_dests[dstSubresource].output_row_pitch * y)), inputRowSize);
