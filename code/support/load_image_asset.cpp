...
static inline uint32_t get_format_aspect_count(PAL_ASSET_IMAGE_FORMAT format);
static inline uint32_t get_format_block_size(PAL_ASSET_IMAGE_FORMAT format);
static inline uint32_t get_format_block_width(PAL_ASSET_IMAGE_FORMAT format);
static inline uint32_t get_format_block_height(PAL_ASSET_IMAGE_FORMAT format);
...
extern size_t load_image_asset_calculate_subresource_memcpy_dests(PAL_ASSET_IMAGE_FORMAT format, uint32_t width, uint32_t height, uint32_t depth, uint32_t mip_levels, uint32_t array_layers, size_t staging_buffer_base_offset, uint32_t staging_buffer_offset_alignment, uint32_t staging_buffer_row_pitch_alignment, uint32_t subresource_count, LOAD_IMAGE_ASSET_SUBRESOURCE_MEMCPY_DEST *subresource_memcpy_dests)
{
    ...
    uint32_t const aspect_count = get_format_aspect_count(format);
    uint32_t const block_size = get_format_block_size(format);
    uint32_t const block_width = get_format_block_width(format);
    uint32_t const block_height = get_format_block_height(format);

    // vkspec: bufferOffset must be a multiple of 4
    staging_buffer_offset_alignment = utils_align_up(utils_align_up(staging_buffer_offset_alignment, 4U), block_size);

    size_t total_bytes = 0U;
    size_t staging_buffer_offset = staging_buffer_base_offset;
    ...
    for (uint32_t aspect_index = 0U; aspect_index < aspect_count; ++aspect_index)
    {
        for (uint32_t array_layer = 0U; array_layer < array_layers; ++array_layer)
        {
            ...
            for (uint32_t mipLevel = 0U; mipLevel < mip_levels; ++mipLevel)
            {
                ...
                uint32_t const output_row_pitch = utils_align_up(output_row_size, staging_buffer_row_pitch_alignment);
                uint32_t const output_slice_pitch = output_row_pitch * output_row_count;

                size_t const new_staging_buffer_offset = utils_align_up(staging_buffer_offset, static_cast<size_t>(staging_buffer_offset_alignment));
                total_bytes += (new_staging_buffer_offset - staging_buffer_offset);
                staging_buffer_offset = new_staging_buffer_offset;

                uint32_t const subresource_index = load_image_asset_calculate_subresource_index(mipLevel, array_layer, aspect_index, mip_levels, array_layers);
                assert(subresource_index < subresource_count);

                subresource_memcpy_dests[subresource_index].staging_buffer_offset = staging_buffer_offset;
                ...
                staging_buffer_offset += surface_size;
                assert((staging_buffer_base_offset + total_bytes) == staging_buffer_offset);
